...
class CommandCenter:
    ...
    def message_received(self, client, server, jsonString):
        ...
        if(message["type"] == "init"):
            ...
            self.procesCommand(message["message"])
        elif(message["type"] == "log"):
            print(message["message"])

    def send_message(self, client: str, message: str):
        for client in self.clients:
            if(client['name'] == client):
                self.server.send_message(client['id'], Message("command", message))
        
    def procesCommand(self, command: str):
        if(command == "play"):
            if(self.music == None):
                    self.music = subprocess.Popen(["python3", "/home/pi/Desktop/raspberry/music-player/main.py"])
            else:
                self.music.kill()
                self.music = None
